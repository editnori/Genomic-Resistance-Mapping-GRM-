...
class CTkScrollbar(CTkBaseClass):
    ...
    def _create_bindings(self, sequence: Optional[str] = None):
        ...
        if sequence is None or sequence == "<ButtonPress-1>":

            def set_press_start(event):
                self._press_start = self._get_scroll_value(event) - self._start_value

            self._canvas.bind("<ButtonPress-1>", set_press_start)
    ...
    def _get_scroll_value(self, event):
        if not event:
            return 0
        position = (event.x, event.y)
        if self._orientation == "vertical":
            return self._reverse_widget_scaling(
                (
                    (position[1] - self._border_spacing)
            ...
            return self._reverse_widget_scaling(
                (
                    (position[0] - self._border_spacing)
    ...
    def _clicked(self, event):
        ...
        value = self._get_scroll_value(event) - self._press_start

        value = min(max(0, value), 1 - current_scrollbar_length)

        self._start_value = value
        self._end_value = value + current_scrollbar_length
